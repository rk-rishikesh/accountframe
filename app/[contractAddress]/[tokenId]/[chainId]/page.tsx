...
export default function Token({ params, searchParams }: TokenParams) {
  ...
  useEffect(() => {
    ...
    if(!isNil(nftImages)) {
      setImag("https://gateway.lighthouse.storage/" + nftImages[0].slice(15));
    };
  ...
  return (
    <div className="h-screen w-screen bg-slate-100">
      <div className="max-w-screen relative mx-auto aspect-square max-h-screen overflow-hidden bg-black">
        <div className="relative h-full w-full">
          {account && nftImages && nftMetadata && (
            <div className="absolute left-4 top-4 z-10 rounded-full cursor-pointer">
              <img className="w-10" src="/apple-touch-icon.png" alt=""/>
            </div>
          )}
          <div className="max-h-1080[px] relative h-full w-full max-w-[1080px]">
            {showLoading ? (
              ...
              <div
                className={`bg-black h-full w-full grid grid-cols-1 grid-rows-1 transition
                  }`}
              >
                {!isNil(nftImages) ? (
                  nftImages.map((image, i) => (
                    <>
                      <div className="bg-cover overflow-x-scroll no-scrollbar items-center justify-center right-0 sm:mt-0">
                        <div
                      ...
                      <div className="absolute mt-20 ml-[45%]">
